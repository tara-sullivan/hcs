...
\toedit{
    I then then use this model to consider how of group-based beliefs influences college major choice by gender. 
    \begin{blist}
    \item Ine line with \textcite{HHJK19}, we know these different beliefs can have significant effects on producitvity. 
    \end{blist}
}
...
\begin{equation}\label{eq:h_eq_alpha_v}
    h_{j0}^g = \nu_j \alpha_{j0}^g.
\end{equation}
Equation \eqref{eq:h_eq_alpha_v} is consistent with the human capital accumulation function in equation \eqref{eq:hc_accumulation}, and allows for an analytical solution to the index and graduation region, \toedit{as described in \textcite{AF20}.}

\toedit{
To see why, let's revisit how the regularity condition \eqref{eq:h_leq_alpha_v} impacts the graduation index \eqref{eq:graduation_general}. 
The graduation index \eqref{eq:graduation_general} characterizes the states for which individuals would stop studying field-$j$ and enter the labor market as a field-$j$ specialist, ignoring all other fields.
Assuming $h_{jt} \leq \alpha_{j0}^g \nu_j$ implies the stopping problem for a given field is monotone.
Here, monotonicity means that if an agent that would choose to stop studying $j$ at time $t$, then they would still want to stop studying $j$ at time $t+1$ if they continued on, independent of stochastic outcomes. 
Monotonicity implies the optimality of a one-step-look-ahead comparison for the field-specific stopping problem.
Therefore, when an agent at time $t$ decides whether to stop studying $j$, they compare their expected lifetime payoff for studying $j$ in the current period with their payoff in the subsequent period.
